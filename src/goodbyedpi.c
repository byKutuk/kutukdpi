...
#define GOODBYEDPI_VERSION "v0.1.0"
...
int main(int argc, char *argv[]) {
    ...
    printf(
        "KutukDPI " GOODBYEDPI_VERSION
        ": Vodafone ve Turk ISP'ler icin DPI atlatma araci (GoodbyeDPI tabanli)\n"
        "https://github.com/byKutuk/kutukdpi\n\n"
    );
    ...
    while ((opt = getopt_long(argc, argv, "123456789pqrsaf:e:mwk:n", long_options, NULL)) != -1) {
        switch (opt) {
            ...
            case 'x': // --debug-exit
                ...
                break;
            default:
                puts("Usage: kutukdpi.exe [OPTION...]\n"
                " -p          block passive DPI\n"
                " -q          block QUIC/HTTP3\n"
    ...
    printf("Filter activated, KutukDPI is now running!\n");
